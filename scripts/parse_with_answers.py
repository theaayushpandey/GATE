...
# --- Pass 1: Parse Answer Keys ---
# Answer keys are at the bottom of the document or at the end of each section
# Format: "DBMS PYQs Answer key:" or "CSO PYQs Answer key:"
# Then topic names, then lines like "1. C 2. D 3. A"
lines = text.split('\n')
answer_keys = {} # dict of subject_name -> topic_name -> q_num -> answer

current_ans_subject = None
current_ans_topic = None
...
for line in lines:
    ...
    # If a new booklet starts, we are no longer in the answer section
    if 'PYQ Questions Booklet' in line_clean or 'PYQ Questions' in line_clean:
        in_answer_section = False
        continue

    # Check if we hit an answer key section
    ...
    if m_header:
        ...
        # Sometimes header is like "Operating System \n PROCESS & THREAD MANAGEMENT \n 1. C"
    ...
    if in_answer_section and current_ans_subject:
        # Check if line contains answers
        ...
        if answers_found:
            ...
            # If no answers found, it might be a topic header
            if len(line_clean) > 3 and not line_clean.isdigit():
                # Avoid capturing random text as topics
...
topic_pattern = re.compile(r'^[A-Z\s&+\-]{5,}$') # All caps line might be a topic
...
for line in lines:
    ...
    # Skip processing questions if we are in the answer key section
    if in_answer_section_pass2:
        # Check if we left the answer key section (rare, they are usually at the end)
        if "PYQ Questions Booklet" in line_clean:
            in_answer_section_pass2 = False
        continue

    # Try to detect subject from booklet headers
    ...
    # Check for topic headers (all caps, no numbers, relatively short)
    ...
    if m_q:
        ...
        if current_subject and current_subject in answer_keys:
            ...
            if current_topic in topic_dict and q_num in topic_dict[current_topic]:
                ...
                # Try finding it in any topic for this subject if exact match fails
...
# Remove the old injected block from seed.sql and append the new one
with open(seed_path, 'r', encoding='utf-8') as f:
    seed_content = f.read()

# Find the marker of the previous injection
idx = seed_content.find('-- ─── INJECTED PDF PYQs ──────────────────────────────────────')
if idx != -1:
    seed_content = seed_content[:idx]

seed_content += "".join(sql_statements)

with open(seed_path, 'w', encoding='utf-8') as f:
    f.write(seed_content)

print(f"Extracted {len(questions)} questions. Successfully mapped correct keys for {mapped_count} questions.")
print("Updated seed.sql")
